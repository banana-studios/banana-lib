...
pub fn get_short_name(full_name: &str) -> String {
    ...
    while index < end_of_string {
        ...
        if let Some(special_character_index) = rest_of_string.find(|c: char| {
            ...
            let segment_to_collapse =
                rest_of_string.get(0..special_character_index).unwrap_or_default();
...
mod name_formatting_tests {
    ...
    #[test]
    fn path_seperated() {
        assert_eq!(get_short_name("bevy_prelude::make_fun_game"), "make_fun_game".to_string());
    }

    #[test]
    fn tuple_type() {
        assert_eq!(get_short_name("(String, String)"), "(String, String)".to_string());
    }
